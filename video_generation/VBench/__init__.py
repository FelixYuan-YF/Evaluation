...
class VBench(object):
    ...
    def _scan_videos_from_directory(self, videos_path, dimension_list, custom_image_folder=None, custom_image_dict=None):
        """Scan videos from a directory and build video info list."""
        cur_full_info_list = []
        ...
        if os.path.isfile(videos_path):
            ...
            if custom_image_folder is not None:
                video_info["custom_image_path"] = custom_image_dict.get(get_prompt_from_filename(videos_path), "")
            cur_full_info_list.append(video_info)
        else:
            video_names = os.listdir(videos_path)
            
            for filename in video_names:
                postfix = Path(os.path.join(videos_path, filename)).suffix
                if postfix.lower() not in ['.mp4', '.gif']:
                    continue
                ...
                if custom_image_folder is not None:
                    video_info["custom_image_path"] = custom_image_dict.get(get_prompt_from_filename(filename), "")
                
                cur_full_info_list.append(video_info)
        
        return cur_full_info_list
    
    def _build_video_map(self, prompt_list, videos_path, cur_full_info_list):
        """Build video path to prompt mapping."""
        # Check if prompt_list keys are absolute paths
        first_key = next(iter(prompt_list.keys())) if prompt_list else ""
        
        if os.path.isabs(first_key):
            # CSV mode: prompt_list already contains absolute paths
            video_map = {}
            for prompt_key in prompt_list:
                video_map[os.path.abspath(prompt_key)] = prompt_list[prompt_key]
            
            # For symlinked videos, also map the symlink paths
            for video_info in cur_full_info_list:
                video_abs_path = os.path.abspath(video_info["video_list"][0])
                if os.path.islink(video_info["video_list"][0]):
                    real_path = os.path.realpath(video_info["video_list"][0])
                    if real_path in video_map and video_abs_path not in video_map:
                        video_map[video_abs_path] = video_map[real_path]
        else:
            # Standard mode: prompt_list keys are relative paths
    ...
    def _validate_and_assign_prompts(self, video_map, cur_full_info_list):
        """Validate that all videos have prompts and assign them."""
        if len(video_map) < len(cur_full_info_list):
            raise AssertionError(f"""
                Number of prompts should match with number of videos.
                Got {len(video_map)} prompts, {len(cur_full_info_list)} videos
        ...
        missing_videos = set(all_video_path) - set(video_map.keys())
    ...
    def build_full_info_json(self, videos_path, name, dimension_list, prompt_list=[],  special_str='', verbose=False, custom_image_folder=None, mode='vbench_standard', **kwargs):
        ...
        if mode=='custom_input':
            ...
            # CSV mode: prompt_list contains absolute paths as keys
            if isinstance(prompt_list, dict) and len(prompt_list) > 0:
                first_key = next(iter(prompt_list.keys()))
                if os.path.isabs(first_key):
                    # CSV mode: use video paths from prompt_list keys directly, no need to scan directory
                    for video_path in prompt_list.keys():
                        if not os.path.exists(video_path):
                            print(f"[WARNING] Video file not found: {video_path}")
                            continue
                        postfix = Path(video_path).suffix
                        if postfix.lower() not in ['.mp4', '.gif']:
                            continue
                        
                        video_info = {
                            "prompt_en": prompt_list[video_path], 
                            "dimension": dimension_list, 
                            "video_list": [video_path]
                        }
                        if custom_image_folder is not None:
                            video_info["custom_image_path"] = custom_image_dict.get(get_prompt_from_filename(video_path), "")
                        
                        cur_full_info_list.append(video_info)
                    
                    # CSV mode complete, skip to save full info
                    videos_path = None  # Not needed
                else:
                    # Standard directory scanning mode
                    cur_full_info_list = self._scan_videos_from_directory(videos_path, dimension_list, custom_image_folder, custom_image_dict)
                    
                    # Match prompts with videos
                    if len(prompt_list) > 0:
                        video_map = self._build_video_map(prompt_list, videos_path, cur_full_info_list)
                        self._validate_and_assign_prompts(video_map, cur_full_info_list)
            else:
                # Standard directory scanning mode (prompt_list is empty or list)
                cur_full_info_list = self._scan_videos_from_directory(videos_path, dimension_list, custom_image_folder, custom_image_dict if custom_image_folder else None)
                
                # Handle single prompt for all videos
                if isinstance(prompt_list, list) and len(prompt_list) == 1:
                    for video_info in cur_full_info_list:
                        video_info["prompt_en"] = prompt_list[0]
